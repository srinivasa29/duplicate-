...
const AdvancedChartsBottomPanel = ({ symbol: propSymbol }) => {
  const { symbol: routeSymbol = 'JINDRILL' } = useParams();
  ...
  const getMetricData = () => {
    ...
    if (finTab === 'Revenue') return (financialData.data.revenue || []).map(d => ({ name: d.year || d.name, value: d.value, color: '#3b82f6' }));
    if (finTab === 'Profit') return (financialData.data.profit || []).map(d => ({ name: d.year || d.name, value: d.value, color: '#10b981' }));
  ...
  return (
    <div className="investor-bottom-panel w-full mt-6">
          ...
          {activeTab === 'Fundamentals' && (
            <div className="fundamentals-tab-rich animate-fade-in">
              ...
              <div className="ft-detailed-layout-grid">
                ...
                {(() => {
                  const q = quoteData || {};
                  const isCrypto = ['CRYPTO', 'CRYPTOCURRENCY'].includes(String(q.type || '').toUpperCase());
                  ...
                  const divYield   = fmtPct((q.dividendYield || 0) * 100);
...
export default AdvancedChartsBottomPanel;
